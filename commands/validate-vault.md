...
specifics. This command is config-driven: it reads the company list, team
roster, and canon files from `hq.config.yml` — nothing here is hardcoded to a
particular company.
...
- `companies` — the allowed values for the `company` frontmatter field.
- `team` — each member's `handle` (allowed values for `owner`) and `initials`.
...
present) `library/`, `crm/`, `distillery/`, `competitive/`, `sales/`,
`operations/`. Scan optional module folders recursively (their content lives in
...
- Required fields by type (see `docs/vault-design.md` for the schemas):
  - **Task:** `tags`, `title`, `status`, `priority`, `owner`, `company`,
    `created`. (`initiative` may be empty for admin/orphan tasks.)
  - **Initiative:** `tags`, `title`, `company`, `status`, `owner`, `started`.
  - **Knowledge:** `tags`, `title`, `company`, `topics`, `created`.
    (`topics` is a required key of list type; an empty list `[]` is valid.)
  - **Log:** `tags`, `title`, `date`, `type`.
  - **Library:** `tags`, `title`, `url`, `company`, `topics`, `created`.
    (`topics` is a required key of list type; an empty list `[]` is valid. `url`
    is required but may be an empty string `""` for pasted-text sources.)
  ...
  - **Content** (`tags` contains `content`): `tags`, `title`, `channel`,
    `status`, `author`, `created`. (`pillar`, `published_date`, `url`,
    `cross_posted_from`, and the analytics fields are optional.)
...
- Config-driven validation:
  - Every `company` value (single or list) is one of `companies` from the
    config. (CRM Contact's `company` is a **wikilink** to another vault file, not
    a config handle — exempt it from this check; it is validated as a wikilink in
    Test 3.)
  - Every `owner` value is one of the team `handle`s from the config. Content's
    `author` is likewise a team handle — validate it against the config the same
    way.
- Required body sections by type (check for `##` headings in the file body):
  ...
  - **Log:** `## Summary`, `## Details`, `## Outcome`.
  - **Library:** `## Raw Content`.
  - **Knowledge:** no required body sections (the body is free-form).
...
`library/` and every optional module. Notes and exemptions:

- **`knowledge/`, `library/`** — may carry subfolders with their own indexes (e.g.
  `knowledge/people/`, `knowledge/portfolio/`, `library/<conference>/`); each is
...
- Collect the set of library item slugs three ways. The slug set is the **flat**
  ...
  2. **Index** — every `[[wikilink]]` target in `library/index.md`, excluding
     links to the module files themselves (`index`, `wiki`) and links that point to
     a subfolder's own `index` (`[[library/<capture>/index]]` is a pointer to a
     self-contained sub-index, not an item pointer).
  3. **Wiki** — every `[[wikilink]]` target in `library/wiki.md` that points to a
     library item file (i.e. resolves to a file in `library/`; ignore links to
     other vault files such as cross-references to `knowledge/` items).
