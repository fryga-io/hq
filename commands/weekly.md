...
- `companies` (multi-company vault only) — used to scope the rollup and for section
  headings where multiple companies share the vault; absent in a single-company vault.
- `team` (multi-operator vault only) — the handles operating the vault; each names a
  `people/` note carrying name and initials. The rollup is written for this team;
  address the reader as "the team" unless the config specifies otherwise. A solo
  vault addresses the single operator.
